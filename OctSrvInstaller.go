...
func main() {
	...
	if _, err := os.Stat(prismDir); os.IsNotExist(err) {
		...
		createShortcut("十月模組包啟動器 Oct Mod Launcher", prismExe, iconPath)
	} else {
...
func createShortcut(name, targetPath, iconPath string) {
...
$Shortcut.Save();`, desktop, name, targetPath, iconPath)

	exec.Command("powershell", "-Command", psScript).Start()

	psScript = fmt.Sprintf(`$WshShell = New-Object -ComObject WScript.Shell;
$Shortcut = $WshShell.CreateShortcut('%s\\%s.lnk');
$Shortcut.TargetPath = '%s';
$Shortcut.IconLocation = '%s';
$Shortcut.Save();`, startMenu, name, targetPath, iconPath)

	exec.Command("powershell", "-Command", psScript).Start()

	fmt.Println("捷徑已建立:", name)
}
